...
namespace aoc2019
{
    public class Node : IEquatable<Node>
    {
        ...
        {
            this.Id = id;
            if (child != null) this.Children = new List<Node> { child };
        }
        ...
        internal int Orbits(int count = 0)
        {
...
K)L";
        ...
        private static Node parse(string example, ref HashSet<Node> nodes)
        {
            ...
            {
                var arr = line.Split(")");
                var node1 = new Node(arr[0]);
                {
                    if (!nodes.TryGetValue(node1, out var node))
                    {
                        nodes.Add(node1);
                    }
                    else
                ...
                if (arr[0] == "COM") com = node1;

                var node2 = new Node(arr[1]);
                {
                    if (!nodes.TryGetValue(node2, out var node))
                    {
                        nodes.Add(node2);
                    }
                    else
