...
namespace aoc2019
{
    ...
    Going from left to right, the digits never decrease; they only ever increase or stay the same (like 111123 or 135679).
            ...
            */
            ...
            Assert.IsTrue(isOK(111111));
            Assert.IsFalse(isOK(223450));
            Assert.IsFalse(isOK(123789));
        }

        private bool isOK(int password)
        {
            if (password < 100000) return false;
            if (password > 999999) return false;
            var arr = password.ToString().Select(x => int.Parse(x.ToString())).ToArray();
            bool adjacent = false;
            for (int i = 0; i < 6; i++)
            {
                if (i > 0)
                {
                    if (arr[i - 1] == arr[i]) adjacent = true;
        ...
        private bool isOK2(string password)
        {
            var arr = password.Select(x => int.Parse(x.ToString())).ToArray();
            bool adjacent = false;
            for (int i = 0; i < 6; i++)
            {
                if (i > 0)
        ...
        public void ExamplePart2()
        {
            ...
            Assert.IsTrue(isOK2(112233.ToString()));
            Assert.IsFalse(isOK2(123444.ToString()));
            Assert.IsTrue(isOK2(111122.ToString()));
        }
