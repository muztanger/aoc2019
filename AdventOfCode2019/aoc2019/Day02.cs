        ...
        public void Part2()
        {
            ...
            for (int noun = 0; noun < 100; noun++)
                ...
                { 
                    ...
                    var memory = new int[init.Length];
                    ...
                    memory[1] = noun; //noun
                    memory[2] = verb; //verb
                    //int[] memory = Enumerable.Range(0, 4096).Select(_ => 0).ToArray();

                    int instruction = memory[0];
                    while (instruction != 99)
                    {
                        switch (instruction)
                        {
                            ...
                            case 99:
                                ...
                                break;
                            default:
                                throw new Exception($"Something went wrong opCode={instruction}");
                        }
                        instruction = memory[instructionPointer];
                    }
                    if (memory[0] == 19690720) throw new Exception($"100 * {noun} + {verb} = {100 * noun + verb}");
                }
        }
